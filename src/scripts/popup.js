...
function resetMembers(members,full){
  members.forEach(m=>{
    m.current=false;
    m.bold=false;
    if(full){
      m.completed=false;
      m.providedUpdate=false;
    }
    m.getCss=function(item){
      if(this.providedUpdate){
        return "completed";
      } 
    ...
    _client.getTeamMembersWithExtendedProperties(projectId, teamId, 100, 0).then((members) => {
      ...
      const startRandomizer = () => {
        ...
        const randomNumber = () => {
          ...
          let notProvidedUpdate = members.filter(member=>{
            return !member.providedUpdate;
          })



          const randomIndex = Math.floor(Math.random() * notProvidedUpdate.length);
          let randomMember=notProvidedUpdate[randomIndex];
          if(randomMember){
            randomMember.bold=true;
            bindMembers(members);
            ...
            if (randomizer++ > 20) {
              ...
              notProvidedUpdate[randomIndex].current=true;
              bindMembers(members);
      ...
      function applyComplete(){
        completedTeamMembers.forEach(ctm=>{
          members.forEach(m=>{
            if (m.identity.id == ctm){
              m.providedUpdate=true;
              bindMembers(members);
            }
          });
        })
